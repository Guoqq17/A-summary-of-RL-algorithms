...
def create_q_network(type, para, name = ''):
    if type == 'q_fcn': # Fully connected neural network
        ...
        x = tl.layers.Dense(n_units = n_hidden_nodes[0], act = eval("tf.nn." + act_funcs[0]), W_init = W_init, b_init = b_init, name = 'hidden_layer_1')(inputs)
        for i in range(1, n_hidden_layers):
            x = tl.layers.Dense(n_units = n_hidden_nodes[i], act = eval("tf.nn." + act_funcs[i]), W_init = W_init, b_init = b_init, name = '_hidden_layer_' + str(i + 1))(x)
